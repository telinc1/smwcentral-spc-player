...
(function () {
	...
	function createSPCPlayerUI() {
		...
		const updateUI = () => {
			...
			if (timer.lastUpdatedUI !== time) {
				...
				playerUI.trackTimeElapsed.innerText = `${Math.floor(progress / 60)}:${
					seconds > 9 ? "" : "0"
				}${seconds}`;
	...
	function dragSPCPlayer(element, handle) {
		...
		function keepInViewport() {
			...
			if (rect.x < 0) {
				setPosition(currentX - rect.x, currentY);
			} else if (rect.right > window.innerWidth) {
				setPosition(currentX - (rect.right - window.innerWidth), currentY);
			}

			if (rect.y < 0) {
				setPosition(currentX, currentY - rect.y);
			} else if (rect.bottom > window.innerHeight) {
				setPosition(currentX, currentY - (rect.bottom - window.innerHeight));
			}
		}
		...
		if (window.innerWidth >= 768) {
			...
			function onTouchMove(e) {
				...
				const touch = e.touches[0];
				setPosition(touch.clientX - startX, touch.clientY - startY);
				keepInViewport();
			}
		...
		return {setPosition, resetPosition, keepInViewport};
	}
